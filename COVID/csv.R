...
WHD <- dplyr::filter(mappable, UTLA_areaName %in% c("Wiltshire", "Hampshire", "Dorset", "Bournemouth, Christchurch and Poole", "Southampton", "Swindon"))
...
tm_shape(WHD) +
  tm_polygons("newCasesBySpecimenDateRollingRate", id="areaName", palette = "-viridis", alpha=0.4, title = "Rolling Rate", breaks=c(0,10,50,100,200,400,1000)) +
  tm_basemap(leaflet::providers$CartoDB) +
  tm_view(set.view=c(-1.803468, 51.0797597, 12))
